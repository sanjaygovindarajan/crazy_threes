...
class GameTest {
    ...
    void setCurrentSuit(){
        Game game = new Game(Arrays.asList("player1", "player2", "player3"));
        game.getDiscard().addCard(new Three('S'));
        game.setCurrentSuit('C');
        assertEquals('C',game.getDiscard().getCard().getCurrentSuit());
        assertEquals('S',game.getDiscard().getCard().getDisplaySuit());
    }
